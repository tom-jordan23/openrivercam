...
|------|-------|--------|---------------|--------|
| **Sukabumi** | Solar (200W panel / 50Ah battery) | ANNKE C1200 PoE, power-cycled | Pi 5 + Witty Pi 5 + G469 | **Deployed.** Two RTK surveys reproduced the same ~99 cm H / 139 cm V check-point noise (vs the 3 cm gate). **Resolved: running on the IPB (Bogor University) total-station survey** — camera config `Fit 6`, applied 2026-06-11, built from IPB data only, 0.037 m RMSE against a 5 cm target, `z_0 = h_ref = 615.0 m`. The April RTK auto-fit salvage (4.61 cm on a 6-GCP subset, `z_0 = 617.065`) is obsolete and must not be mixed in — the IPB low-water surface is ~2 m lower. See `survey_data/ipb_survey_1/handoff_station/`. |
| **Jakarta** | AC utility (Mean Well PSU) + APC 900VA UPS | ANNKE C1200 PoE, continuous | Pi 5 + Witty Pi 5 + G469 | **Not deployed.** Permission for the intended site fell through during the trip. Station is built and software-ready, held at **Wisma PMI, Jakarta**; not powered on since April 2026. **Intended as a lab / study and test unit for IPB or BHLK** — local test installation is fine, but not operational service with availability expectations; it carries the design the replication report recommends changing. |
